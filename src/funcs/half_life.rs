...
pub fn half_life(inputs: &[Series], kwargs: HalfLifeKwargs) -> PolarsResult<Series> {
    ...
    let res = match s.dtype() {
        DataType::Int32 => s.i32()?.half_life(kwargs.min_periods),
        DataType::Int64 => s.i64()?.half_life(kwargs.min_periods),
        DataType::Float32 => s.f32()?.half_life(kwargs.min_periods),
        DataType::Float64 => s.f64()?.half_life(kwargs.min_periods),
        dtype => {
